        ...
        IF(NCOMP.GT.0)THEN
          DO 10 I=1,NCOMP
            IF(COMP(1,I).EQ.1)THEN
              ...
              COMP(3,I)=COMP(3,IP)*(100./30.48)*(100./30.48)
...
C
C
C      DATUM=POUT(13,1)-POUT(14,1)
C      WRITE(TBUNT,210) XSNAME,RFOOT,DATUM
      DO 10 I=1,NOUT
...
C
      DO 10 I=1,NOUT
        WRITE(TBUNT,210) POUT(13,I),POUT(3,I),POUT(2,I)
 10   CONTINUE
C
      RETURN
      END
...
C
 ...
 10   CONTINUE
      ...
      IF(ONCHR.EQ.','.OR.ONCHR.EQ. ' '.OR.ONCHR.EQ.'*') THEN
        ILEN=I-BGN
        IF(ONCHR.EQ.','.OR.ONCHR.EQ.'*'.OR.ILEN.GT.0) NCNT=NCNT+1
        IF(ILEN.GT.0) THEN
...
C
C     * * * LOCAL VARIABLES * * *
      INTEGER ISA,ISA1,I,ISA0,FLG,IXMAX,IXMIN,ISUB(19),CLOSED
     #     ,IFLAT,KSUB
      REAL EWL,EWR,PERIMT,D1,S1,G1,Z,SUMH(19),XRIGHT,XLEFT,
     #     D0,S0,G0,DS,DG,B,A,P,H,EW,IMOVE,SWID(19)
C
C     + + + EXTERNALS + + +
